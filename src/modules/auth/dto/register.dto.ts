...
export class RegisterDto {
  ...
  @ApiProperty({
    ...
    description:
      'Min 15 chars, must contain uppercase, lowercase, number, special character',
    minLength: 15,
    maxLength: 128,
  })
  @IsString()
  ...
  @ApiPropertyOptional({
    example: 'player',
    enum: ['player', 'manager'],
    description: 'Only player or manager allowed',
  })
  @IsOptional()
  @IsIn(['player', 'manager'], {
    message: 'Role must be either player or manager',
  })
  @Transform(({ value }: { value: any }) => value?.toLowerCase().trim())
  role?: string;
}
